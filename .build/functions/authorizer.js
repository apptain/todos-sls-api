...
var region = process.env.USER_POOL_REGION;
...
exports.handler = function (event, context, callback) {
    ...
    if (process.env.ENVIRONMENT_TYPE !== 'Production') {
        if (event.authorizationToken.startsWith('TOKEN')) {
            ...
            var policyToReturn = policy.build();
    ...
    if (!pems) {
        request({
            ...
            if (!error && response.statusCode === 200) {
                ...
                console.error("Failed to download jwks.json: " + error);
                context.fail('Error');
            }
        });
    }
        ...
        console.info('Using cached pems');
        validateToken(pems, event, context);
    }
};
var validateToken = function (pems, event, context) {
    var token = event.authorizationToken;
    // Fail if the token is not jwt
    var decodedJwt = jwt.decode(token, { complete: true });
    if (!decodedJwt) {
        console.error("Not a valid JWT token: " + token);
        context.fail('Unauthorized');
        return;
    }
    ...
    if (decodedJwt.payload.token_use != 'id') {
        console.error("Not an id token: " + decodedJwt.payload.token_use);
        context.fail('Unauthorized');
        return;
    }
    ...
    var pem = pems[kid];
    if (!pem) {
        console.error("Invalid id token: " + decodedJwt.header.kid);
        context.fail('Unauthorized');
        return;
    }
